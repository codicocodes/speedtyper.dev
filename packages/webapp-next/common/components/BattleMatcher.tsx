...
export const BattleMatcher: React.FC = () => {
  ...
  const races = data; // data.filter((race) => race.id != raceID);
  return (
...
const BatteListItem: React.FC<BatteListItemProps> = ({ race, closeModal }) => {
  const { username } = race.members[race.owner];
  const membersCount = Object.keys(race.members).length;
    ...
    >
      <div className="flex">
        <div className="flex items-center bg-purple-300 px-2 rounded mr-2">
          <UserGroupIcon /> <span className="mx-2">{membersCount}</span>
        </div>
        <span>{username}</span>
      </div>
